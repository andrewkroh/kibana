...
export const ccLines = (x) => !coverageDelimRe.test(x);
export const ownerLines = (x) => coverageDelimRe.test(x);
...
export const ownerLines$ = (rl) =>
  fromEvent(rl, 'line').pipe(filter(empties), filter(comments), filter(ccLines));

export const coverageLine$ = (rl) =>
  fromEvent(rl, 'line').pipe(filter(ownerLines), map(dropCCDelim));
